...
def escanear_texto(texto: str, nombres=None) -> list[tuple[int, str, str]]:
    with tempfile.NamedTemporaryFile("w", suffix=".md", delete=False,
                                     encoding="utf-8") as f:
        f.write(texto)
        ruta = Path(f.name)
    try:
        return esc.escanear(ruta, nombres or [])
    finally:
        ruta.unlink(missing_ok=True)


def tipos(hallazgos) -> set[str]:
    return {t for _, t, _ in hallazgos}
...
class TestNoRuido(unittest.TestCase):
    ...
    def test_montos_en_pesos(self):
        ...
        self.assertEqual(h, [], f"montos reportados como PII: {h}")
    ...
    def test_aritmetica_en_prosa(self):
        ...
        self.assertEqual(h, [], f"aritmética reportada como PII: {h}")

    def test_cifras_en_uvt(self):
        h = escanear_texto("El tope conjunto es de 1.340 UVT y la exención 2.880 UVT.")
        self.assertEqual(h, [])
...
class TestRepositorio(unittest.TestCase):
    ...
    def test_repo_limpio(self):
        globs = esc.globs_ignorados(RAIZ)
        sucios = []
        for archivo in RAIZ.rglob("*"):
            if not archivo.is_file() or archivo.suffix.lower() not in esc.EXTENSIONES:
                continue
            if esc.IGNORAR & set(archivo.parts):
                continue
            relativo = archivo.relative_to(RAIZ)
            if esc.esta_ignorado(relativo, globs):
                continue
            hallazgos = esc.escanear(archivo, [])
            if hallazgos:
